...
export default function Edit({ title, user, car_rental, car_categories, destinations, drivers }: EditCarRentalProps) {
    const { data, setData, put, processing, errors } = useForm({
        car_category_id: car_rental.carCategory.id.toString(),
        customer_name: car_rental.customer_name,
        customer_email: car_rental.customer_email,
        customer_phone: car_rental.customer_phone,
        customer_address: car_rental.customer_address || '',
        start_date: car_rental.start_date,
        end_date: car_rental.end_date,
        start_time: car_rental.start_time || '09:00',
        end_time: car_rental.end_time || '18:00',
        pickup_location: car_rental.pickup_location,
        dropoff_location: car_rental.dropoff_location || '',
        destination_details: car_rental.destination_details || '',
        distance_km: car_rental.distance_km?.toString() || '',
        special_requests: car_rental.special_requests || '',
        status: car_rental.status,
        payment_status: car_rental.payment_status,
        payment_method: car_rental.payment_method,
        assigned_driver: car_rental.assigned_driver?.toString() || '',
        vehicle_number: car_rental.vehicle_number || '',
        internal_notes: car_rental.internal_notes || '',
        whatsapp_notification: car_rental.whatsapp_notification as boolean,
        email_notification: car_rental.email_notification as boolean,
        sms_notification: car_rental.sms_notification as boolean,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/car-rentals/${car_rental.id}`);
    };
    ...
    return (
        <AdminLayout title={title}>
            <Head title={`Edit Car Rental - ${car_rental.booking_number}`} />
                ...
                <Group justify="space-between" mb="xl">
                    <Group gap={8}>
                        ...
                        <Text size="lg" fw={700} style={{ color: 'rgba(255,255,255,0.9)', letterSpacing: '0.02em' }}>
                            Edit Car Rental - {car_rental.booking_number}
                        </Text>
                    </Group>
                    <Button
                        component={Link}
