...
export default function EditRole({ title, target_role, grouped_permissions }: EditRoleProps) {
    const currentPermIds = target_role.permissions.map((p) => p.id);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/roles/${target_role.id}`);
    };
    ...
    return (
        <AdminLayout title={title}>
            <Head title={`Edit Role — ${target_role.display_name}`} />
                ...
                <Paper p="xl" radius="md" style={{ background: '#11111199', border: '1px solid rgba(255,255,255,0.06)', backdropFilter: 'blur(12px)', maxWidth: 720, width: '100%' }}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <Stack gap="md">
                            ...
                            <Divider
                                label={<Text size="xs" style={{ color: 'rgba(255,255,255,0.35)', letterSpacing: '0.1em' }}>PERMISSION MATRIX</Text>}
                                labelPosition="left"
